...
// Estructura para almacenar posiciones del laberinto
class position_t {
public:
  int i; // fila
  int j; // columna
  
  position_t(int row = 0, int col = 0) : i(row), j(col) {}
  
  // Operador de igualdad para comparar posiciones
...
private:
  ...
  // Vector solucion
  vector<position_t> solution_;
  vector<int> blocked = {0,0,0,0};
