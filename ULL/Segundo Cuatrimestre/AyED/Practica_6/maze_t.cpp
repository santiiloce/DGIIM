...
{ 
  ...
  else if(matrix_.at(i,j) == WALL_ID){
    return false;
  }
  else if(visited_.at(i,j) == true){
    return false;
  }
  return true;
}
...
maze_t::solve_(const int i, const int j)
{
  ...
  // Probar las 4 direcciones: N, E, S, W
  // N
  int s = 0;
  int n = 0;
  int o = 0; 
...
// N
   if (is_ok_(i - 1, j )) {
     if (solve_(i - 1, j )) {
       matrix_.at(i - 1, j ) = PATH_ID;
       solution_.push_back(position_t(i-1,j));
       return true;
     }
     else{
      n++;
  ...
  // O
   if (is_ok_(i  , j - 1)) {
     if (solve_(i  , j - 1)) {
       matrix_.at(i  , j - 1) = PATH_ID;
       solution_.push_back(position_t(i,j-1));
       return true;
     }
   }
   else{
...
// E
   ...
   if(s+n+e+o == 4){
    std::cout << "Estoy confinado" << " en la coordenada (" << i << "," << j << ")" << std::endl;
  }
